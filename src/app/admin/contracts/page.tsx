...
export default async function ContractsPage() {
  ...
  if (!isPrivilegedAdmin(session.role)) {
    return (
      <AccessDenied message="You do not have permission to view contracts." />
    );
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Current contract can be replaced; each vendor stores the version they
        signed.
      </p>
      <form
        action={addContract}
        className="mt-6 grid gap-3 rounded-lg border border-slate-200 bg-white p-5 md:grid-cols-[180px_1fr_auto_auto]"
      >
        <input
          name="version"
          ...
          rows={contracts.map((contract) => [
            ...
            <form key="toggle" action={toggleContractStatus}>
              ...
              <input
                type="hidden"
                name="active"
                value={contract.active ? "false" : "true"}
              />
              <button
                className={
                  contract.active
