...
export default async function KycDetailPage(
  props: PageProps<"/admin/kyc/[id]">,
) {
  const session = await requireAdminSession();
  const privileged = isPrivilegedAdmin(session.role);
  const { id } = await props.params;
  const vendor = await prisma.vendor.findUnique({
    where: { id },
    include: { documents: true, signedContract: true },
  });
  if (!vendor) notFound();
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Contract version: {vendor.signedContract?.version ?? "Not signed"}
      </p>
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-[1fr_auto_auto]">
            ...
            <button
              name="status"
              value="REJECTED"
              className="rounded-md bg-rose-700 px-4 py-2 text-sm font-bold text-white hover:bg-rose-600"
            >
              Reject KYC
            </button>
          </div>
      ...
      {!privileged &&
        (vendor.kycStatus === "PENDING" || vendor.kycStatus === "REVIEW") && (
          <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-5 text-amber-800">
            You can view this KYC record, but only managers and superadmins may
      ...
      <div className="mt-6">
        <DataTable
          ...
          rows={vendor.documents.map((doc) => [
            ...
            <DocumentPreview
              key={doc.id}
              label={`${vendor.businessName} ${doc.type}`}
              url={doc.fileUrl}
            />,
