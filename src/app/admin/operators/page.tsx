...
export default async function OperatorsPage() {
  ...
  if (!isPrivilegedAdmin(session.role)) {
    return (
      <AccessDenied message="You do not have permission to view operator management." />
    );
  }

  const operators = await prisma.operator.findMany({
    include: { _count: { select: { sims: true, orders: true } } },
    orderBy: { name: "asc" },
  });
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Only one operator needs to be active now; all records are already
        operator-tagged.
      </p>
      <div className="mt-6">
        <DataTable
          ...
          rows={operators.map((operator) => [
            <span className="font-bold text-slate-950" key={operator.id}>
              {operator.name}
            </span>,
            <form
              key="commission"
              action={updateOperatorDefaults}
            ...
            >
              ...
              <input
                ...
                type="number"
                min="0"
                defaultValue={operator.defaultCommission}
                className="w-24 rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
              <input
                name="defaultFine"
                type="hidden"
                value={operator.defaultFine}
              />
            ...
            >
              ...
              <input
                name="defaultCommission"
                type="hidden"
                value={operator.defaultCommission}
              />
              <input
                name="defaultFine"
                type="number"
                min="0"
                defaultValue={operator.defaultFine}
                className="w-24 rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
              <button className="rounded-md bg-slate-950 px-3 py-2 text-xs font-bold text-white hover:bg-slate-800">
                Update
            ...
            <form key="toggle" action={toggleOperatorStatus}>
              ...
              <input
                type="hidden"
                name="status"
                value={operator.status === "ACTIVE" ? "INACTIVE" : "ACTIVE"}
              />
              <button
                className={
