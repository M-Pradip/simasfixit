...
export default async function VendorsPage() {
  ...
  if (!isPrivilegedAdmin(session.role)) {
    return (
      <AccessDenied message="You do not have permission to view vendors." />
    );
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Onboarding, KYC status, rates, notes, and order lock state.
      </p>
      <div className="mt-6">
        <DataTable
          headers={[
          ...
          rows={vendors.map((vendor) => [
            ...
            vendor.balance < 0 ? (
              <StatusBadge key="blocked" status="REJECTED" />
            ) : (
              <StatusBadge key="open" status="ACTIVE" />
            ),
          ])}
        />
      </div>
