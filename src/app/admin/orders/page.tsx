...
export default async function OrdersPage(props: {
  ...
  if (!isPrivilegedAdmin(session.role)) {
    return (
      <AccessDenied message="You do not have permission to view orders." />
    );
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Assign SIM ranges and move orders through pending, assigned, dispatched,
        delivered.
      </p>
      ...
      <div className="mt-6">
        <DataTable
          ...
          rows={orders.map((order) => [
            ...
            order.operator.name,
            order.quantity,
            order.assignedStartNumber
              ? `${order.assignedStartNumber}-${order.assignedEndNumber}`
              : "Not assigned",
            order.paymentMethod,
